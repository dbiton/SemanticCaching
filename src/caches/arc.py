...
class ARC(Cache):
    def __init__(self, same_embed_distance):
        ...
        self.items = set()               # <-- track all actually stored items

    # ------------------------------------------------------------------ #
    # helpers
    def _add_to_T1(self, eid, vec):
        self.T1[eid] = vec
        self.add_with_ids(vec[np.newaxis, :], np.array([eid], dtype=np.int64))
        self.items.add(eid)  # <-- track insert
    ...
    def _move_T1_to_T2(self, eid):
        vec = self.T1.pop(eid)
        self.T2[eid] = vec                    # already in FAISS; no change to self.items
    ...
    def _replace(self, incoming_id):
        ...
        self.items.discard(vid)  # <-- track eviction
        return vid
    # ------------------------------------------------------------------ #
    ...
    def cache(self, embeds, embeds_ids, count_nn=1, texts=[]):
        ...
        for vec, eid, hit in zip(embeds, embeds_ids, cache_hits):
            ...
            if eid in self.B1:                           # Case III
                ...
                self._add_to_T2(eid, vec)               # <-- adds to items
                continue
            ...
            self._add_to_T1(eid, vec)                   # <-- adds to items
